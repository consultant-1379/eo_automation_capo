...
for (def job_name: [
        'eo-rv-capo-upgrade',
        'eo-rv-capo-upgrade-debug'
    ])

{
    freeStyleJob(job_name) {
        description Defaults.description("""<h2>Job for CAPO upgrade</h2>""")
        ...
        parameters {
            choiceParam('ENV', Defaults.capoEnvList, 'Choose the environment you want to upgrade')
            choiceParam('CONTROLLER_ENV', Defaults.controllersList, 'The server that is used to execute procedures')
            stringParam('ECCD_LINK', 'https://arm.sero.gic.ericsson.se/artifactory/proj-erikube-generic-local/erikube/releases/2.25.0/CXP9043015/CXP9043015R1B.tgz', 'Provide link to download CAPO version.')
            ...
            stringParam('WORKER_DIMENSIONS', '6, 18, 25, 116', 'Provide info about worker node from CPI.<br> Node Count, VCPUs per node, Memory per node (GB), Root Volume per node (GB)')
