...
for ( def job_name : [
        'eo-rv-ccm-installation',
        'eo-rv-ccm-installation-debug'
        ]
    )
{
    freeStyleJob(job_name) {
        ...
        parameters {
            choiceParam('ENV', Defaults.cmEnvsList, 'Choose the environment you want to install')
            ...
            if (job_name.contains('debug')) {
                ...
                booleanParam('INSTALL_CLM', false, 'True to install CLM')
